<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $test_groups->total() }}">
            <button type="button"
            ...
            @can('Test Group Add')
                <a class="btn btn-primary" href="{{ route('test-group.create') }}">
                    <i class="mdi mdi-plus"></i> Add Test Group
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($test_groups as $test_group)
                            <tr>
                                <td scope="row">{{ $test_groups->firstItem() + $loop->index }}</td>
                                <td>{{ $test_group->name }}</td>
                                <td>{{ $test_group->description }}</td>
                                <td><x-status :status="$test_group->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Test Group View')
                                            <li>
                                                <a href="{{ route('test-group.show', $test_group->id) }}"
                                                    class="dropdown-item">
                                            ...
                                            @can('Test Group Edit')
                                            <li>
                                                <a href="{{ route('test-group.edit', $test_group->id) }}"
                                            ...
                                            @endcan
                                            @can('Test Group Delete')
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $test_group->id }})'>
