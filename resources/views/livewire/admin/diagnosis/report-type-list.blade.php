<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $report_types->total() }}">
            <button type="button"
            ...
            @can('Report Type Add')
                <a class="btn btn-primary" href="{{ route('report-type.create') }}">
                    <i class="mdi mdi-plus"></i> Add Report Type
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($report_types as $report_type)
                            <tr>
                                <td scope="row">{{ $report_types->firstItem() + $loop->index }}</td>
                                <td>{{ $report_type->name }}</td>
                                <td><x-status :status="$report_type->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Report Type View')
                                                <li>
                                                    <a href="{{ route('report-type.show', $report_type->id) }}"
                                                        class="dropdown-item">
                                            ...
                                            @can('Report Type Edit')
                                                <li>
                                                    <a href="{{ route('report-type.edit', $report_type->id) }}"
                                            ...
                                            @endcan
                                            @can('Report Type Delete')
                                                <li>
                                                    <a type="button" class="dropdown-item remove-item-btn"
                                                        wire:click='$dispatch("delete_resource",{{ $report_type->id }})'>
