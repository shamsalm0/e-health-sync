<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $test_attributes->total() }}">
            <button type="button"
            ...
            @can('Test Attribute Add')
                <a class="btn btn-primary" href="{{ route('test-attribute.create') }}">
                    <i class="mdi mdi-plus"></i> Add Test Attribute
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($test_attributes as $test_attribute)
                            <tr>
                                <td scope="row">{{ $test_attributes->firstItem() + $loop->index }}</td>
                                <td>{{ $test_attribute->name }}</td>
                                <td><x-status :status="$test_attribute->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Test Attribute View')
                                                <li>
                                                    <a href="{{ route('test-attribute.show', $test_attribute->id) }}"
                                                        class="dropdown-item">
                                            ...
                                            @endcan
                                            @can('Test Attribute Edit')
                                                <li>
                                                    <a href="{{ route('test-attribute.edit', $test_attribute->id) }}"
                                            ...
                                            @endcan
                                            @can('Test Attribute Delete')
                                                <li>
                                                    <a type="button" class="dropdown-item remove-item-btn"
                                                        wire:click='$dispatch("delete_resource",{{ $test_attribute->id }})'>
