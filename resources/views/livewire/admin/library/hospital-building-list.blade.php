<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $buildings->total() }}">
            <button type="button"
            ...
            @can('Hospital Building Add')
                <a class="btn btn-primary" href="{{ route('hospital-building.create') }}">
                    <i class="mdi mdi-plus"></i> Add Building
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($buildings as $building)
                            <tr>
                                <td scope="row">{{ $buildings->firstItem() + $loop->index }}</td>
                                <td>{{ $building->name }}</td>
                                <td>{{ $building->phone }}</td>
                                <td>{{ $building->email }}</td>
                                <td>{{ $building->address }}</td>
                                <td><x-status :status="$building->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            ...
                                            <li>
                                                <a href="{{ route('hospital-building.show', $building->id) }}" class="dropdown-item">
                                                    <i class="ri-eye-fill align-bottom me-2 text-muted"></i>
                                                    View
                                                </a>
                                            </li>
                                            @endcan
                                            @can('Hospital Building Edit')
                                            <li>
                                                <a href="{{ route('hospital-building.edit', $building->id) }}"
                                            ...
                                            @endcan
                                            @can('Hospital Building Delete')
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $building->id }})'>
