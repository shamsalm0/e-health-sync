<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $unions->total() }}">
            <button type="button"
            ...
            @can('Union Add')
                <a class="btn btn-primary" href="{{ route('union.create') }}">
                    <i class="mdi mdi-plus"></i> Add Union
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($unions as $union)
                            <tr>
                                <td scope="row">{{ $unions->firstItem() + $loop->index }}</td>
                                <td>{{ $union->name }}</td>
                                <td>{{ $union->bn_name }}</td>
                                <td>{{ $union->upazila?->name }}</td>
                                <td>{{ $union->url }}</td>
                                <td><x-status :status="$union->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Union View')
                                                <li>
                                                    <a href="{{ route('union.show', $union->id) }}"
                                                        class="dropdown-item">
                                            ...
                                            @can('Union Delete')
                                                <li>
                                                    <a type="button" class="dropdown-item remove-item-btn"
                                                        wire:click='$dispatch("delete_resource",{{ $union->id }})'>
