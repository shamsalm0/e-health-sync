<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $hospitals->total() }}">
            <button type="button"
            ...
            @can('Hospital Add')
                <a class="btn btn-primary" href="{{ route('hospital.create') }}">
                    <i class="mdi mdi-plus"></i> Add Hospital
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($hospitals as $hospital)
                            <tr>
                                <td scope="row">{{ $hospitals->firstItem() + $loop->index }}</td>
                                <td>{{ $hospital->name }}</td>
                                <td>{{ $hospital->mobile }}</td>
                                <td>{{ $hospital->phone }}</td>
                                <td>{{ $hospital->email }}</td>
                                <td>{{ $hospital->address }}</td>
                                <td><x-status :status="$hospital->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Hospital View')
                                            <li>
                                                <a href="{{ route('hospital.show', $hospital->id) }}" class="dropdown-item">
                                            ...
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $hospital->id }})'>
