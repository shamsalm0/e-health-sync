<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $floors->total() }}">
            <button type="button"
            ...
            @can('Hospital Floor Add')
                <a class="btn btn-primary" href="{{ route('hospital-floor.create') }}">
                    <i class="mdi mdi-plus"></i> Add Floor
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($floors as $floor)
                            <tr>
                                <td scope="row">{{ $floors->firstItem() + $loop->index }}</td>
                                <td>{{ $floor->branch?->name }}</td>
                                <td>{{ $floor->building?->name }}</td>
                                <td>{{ $floor->name }}</td>
                                <td><x-status :status="$floor->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            ...
                                            <li>
                                                <a href="{{ route('hospital-floor.show', $floor->id) }}" class="dropdown-item">
                                                    <i class="ri-eye-fill align-bottom me-2 text-muted"></i>
                                                    View
                                                </a>
                                            </li>
                                            @endcan
                                            @can('Hospital Floor Edit')
                                            <li>
                                                <a href="{{ route('hospital-floor.edit', $floor->id) }}"
                                            ...
                                            @endcan
                                            @can('Hospital Floor Delete')
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $floor->id }})'>
