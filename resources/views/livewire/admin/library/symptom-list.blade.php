<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $symptoms->total() }}">
            <button type="button"
            ...
            @can('Symptom Add')
                <a class="btn btn-primary" href="{{ route('symptom.create') }}">
                    <i class="mdi mdi-plus"></i> Add Symptom
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($symptoms as $symptom)
                            <tr>
                                <td scope="row">{{ $symptoms->firstItem() + $loop->index }}</td>
                                <td>{{ $symptom->name }}</td>
                                <td><x-status :status="$symptom->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Symptom View')
                                            <li>
                                                <a href="{{ route('symptom.show', $symptom->id) }}" class="dropdown-item">
                                            ...
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $symptom->id }})'>
