<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $shifts->total() }}">
            <button type="button"
            ...
            @can('Shift Add')
                <a class="btn btn-primary" href="{{ route('shift.create') }}">
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($shifts as $shift)
                            <tr>
                                <td scope="row">{{ $shifts->firstItem() + $loop->index }}</td>
                                <td>{{ $shift->branch?->name }}</td>
                                <td>{{ $shift->name }}</td>
                                <td>{{ $shift->from }}</td>
                                <td>{{ $shift->to }}</td>
                                <td><x-status :status="$shift->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Shift View')
                                            <li>
                                                <a href="{{ route('shift.show', $shift->id) }}" class="dropdown-item">
                                            ...
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $shift->id }})'>
