<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $occupations->total() }}">
            <button type="button"
            ...
            @can('Occupation Add')
                <a class="btn btn-primary" href="{{ route('occupation.create') }}">
                    <i class="mdi mdi-plus"></i> Add Occupation
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($occupations as $occupation)
                            <tr>
                                <td scope="row">{{ $occupations->firstItem() + $loop->index }}</td>
                                <td>{{ $occupation->name }}</td>
                                <td><x-status :status="$occupation->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Occupation View')
                                            <li>
                                                <a href="{{ route('occupation.show', $occupation->id) }}" class="dropdown-item">
                                            ...
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $occupation->id }})'>
