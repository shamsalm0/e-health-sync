<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $banks->total() }}">
            <button type="button"
            ...
            @can('Bank Add')
                <a class="btn btn-primary" href="{{ route('bank.create') }}">
                    <i class="mdi mdi-plus"></i> Add Bank
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($banks as $bank)
                            <tr>
                                <td scope="row">{{ $banks->firstItem() + $loop->index }}</td>
                                <td>{{ $bank->name }}</td>
                                <td><x-status :status="$bank->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Bank View')
                                            <li>
                                                <a href="{{ route('bank.show', $bank->id) }}" class="dropdown-item">
                                            ...
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $bank->id }})'>
