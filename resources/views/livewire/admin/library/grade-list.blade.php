<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $grades->total() }}">
            <button type="button"
            ...
            @can('Grade Add')
                <a class="btn btn-primary" href="{{ route('grades.create') }}">
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($grades as $grade)
                            <tr>
                                <td scope="row">{{ $grades->firstItem() + $loop->index }}</td>
                                <td>{{ $grade->name }}</td>
                                <td>{{ $grade->description }}</td>
                                <td><x-status :status="$grade->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            @can('Grade View')
                                            <li>
                                                <a href="{{ route('grades.show', $grade->id) }}" class="dropdown-item">
                                            ...
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $grade->id }})'>
