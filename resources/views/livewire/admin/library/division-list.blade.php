<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $divisions->total() }}">
            <button type="button"
            ...
            @can('Division Add')
                <a class="btn btn-primary" href="{{ route('division.create') }}">
                    <i class="mdi mdi-plus"></i> Add Division
                </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($divisions as $division)
                            <tr>
                                <td scope="row">{{ $divisions->firstItem() + $loop->index }}</td>
                                <td>{{ $division->name }}</td>
                                <td>{{ $division->bn_name }}</td>
                                <td>{{ $division->url }}</td>
                                <td><x-status :status="$division->status" /></td>
                                <td>
                                            ...
                                            @can('Division Delete')
                                                <li>
                                                    <a type="button" class="dropdown-item remove-item-btn"
                                                        wire:click='$dispatch("delete_resource",{{ $division->id }})'>
