                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="flex-shrink-0">
                            <div class="d-flex flex-wrap gap-2 align-items-center">
                                <h5 class="card-title mb-0">Total: {{ $products->total() }}</h5>
                                @can('Store Product Add')
                                    <a class="btn btn-success add-btn custom-pointer" href="{{ route('store.product.create') }}"><i
                        ...
                        <table class="table table-hover table-striped align-middle table-nowrap table-sm"
                            ...
                            <tbody>
                                @foreach ($products as $product)
                                    <tr>
                                        <th scope="row" class="text-center">{{ ($products->currentPage() - 1) * $products->perPage() + $loop->iteration }}</th>
                                        <td>{{ $product->name }}</td>
                                        <td>{{ $product->short_name }}</td>
                                        <td>{{ $product->code }}</td>
                                        <td>{{ $product->category?->name }}</td>
                                        <td>{{ $product->is_lab ? 'Yes' : 'No' }}</td>
                                        <td>{{ $product->type }}</td>
                                            ...
                                            <div class="d-flex justify-content-center align-items-center">
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" type="checkbox" role="switch"
                                                        ...
                                                        wire:click="@can('Store Product Edit') toggleStatus({{ $product->id }}) @endcan"
                                                        @if ($product->status == 1) checked @endif
                                                        @cannot('Store Product Edit') disabled @endcannot>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <div class="dropdown d-inline-block">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    ...
                                                    <li>
                                                        @can('Store Product Delete')
                                                            <a class="dropdown-item remove-item-btn custom-pointer"
                                                                wire:click='$dispatch("delete_resource",{{ $product->id }})'>
                                                                <i
