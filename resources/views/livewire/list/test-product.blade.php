<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $testProducts->total() }}">
            <button type="button"
            ...
            @can('Test Product Add')
            <a class="btn btn-primary" href="{{ route('test-product.create') }}">
                <i class="mdi mdi-plus"></i> Add Test Product
            </a>
            @endcan
        </x-list-header>
                ...
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($testProducts as $testProduct)
                            <tr>
                                <td scope="row">{{ $testProducts->firstItem() + $loop->index }}</td>
                                <td>{{ $testProduct->testName?->name }}</td>
                                <td>{{ $testProduct->storeProduct?->name }}</td>
                                <td><x-status :status="$testProduct->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            ...
                                            <li>
                                                <a href="{{ route('test-product.show', $testProduct->id) }}"
                                                    class="dropdown-item">
                                            ...
                                            @endcan
                                            @can('Test Product Edit')
                                            <li>
                                                <a href="{{ route('test-product.edit', $testProduct->id) }}"
                                            ...
                                            @endcan
                                            @can('Test Product Delete')
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $testProduct->id }})'>
