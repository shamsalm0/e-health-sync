<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $ticketFees->total() }}">
            <button type="button"
            ...
            @can('Ticket Fee Add')
                <a class="btn btn-primary" href="{{ route('ticket-fee.create') }}">
                    <i class="mdi mdi-plus"></i> Add Ticket Fee
                </a>
            @endcan
        </x-list-header>
        ...
        <div class="card mb-3">
            <div class="card-body">
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($ticketFees as $ticketFee)
                            <tr>
                                <td scope="row">{{ $ticketFees->firstItem() + $loop->index }}</td>
                                @php
                                    $type = '';
                                    if ($ticketFee->type == 1) {
                                ...
                                @endphp
                                <th>{{ $type }}</th>
                                <td>{{ $ticketFee->price }}</td>
                                <td><x-status :status="$ticketFee->status" /></td>
                                <td>
                                    @canany(['Ticket Fee View', 'Ticket Fee Edit', 'Ticket Fee Delete'])
                                        <div class="dropdown d-inline-block">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                @can('Ticket Fee View')
                                                    <li>
                                                        <a href="{{ route('ticket-fee.show', $ticketFee->id) }}"
                                                            class="dropdown-item">
                                                ...
                                                @can('Ticket Fee Edit')
                                                    <li>
                                                        <a href="{{ route('ticket-fee.edit', $ticketFee->id) }}"
                                                ...
                                                @endcan
                                                @can('Ticket Fee Delete')
                                                    <li>
                                                        <a type="button" class="dropdown-item remove-item-btn"
                                                            wire:click='$dispatch("delete_resource",{{ $ticketFee->id }})'>
