<div>
    ...
    <div class="mb-3 ">
        ...
        <x-list-header total="{{ $testMachines->total() }}">
            <button type="button"
            ...
            @can('Test Machine Add')
            <a class="btn btn-primary" href="{{ route('test-machine.create') }}">
                <i class="mdi mdi-plus"></i> Add Test Machine
            </a>
            @endcan
        </x-list-header>
        ...
        <div class="card mb-3">
            <div class="card-body">
                <x-table>
                    ...
                    <x-tbody>
                        @foreach ($testMachines as $testMachine)
                            <tr>
                                <td scope="row">{{ $testMachines->firstItem() + $loop->index }}</td>
                                <td>{{ $testMachine->testName?->name }}</td>
                                <td>{{ $testMachine->machine?->name }}</td>
                                <td><x-status :status="$testMachine->status" /></td>
                                <td>
                                    <div class="dropdown d-inline-block">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            ...
                                            <li>
                                                <a href="{{ route('test-machine.show', $testMachine->id) }}"
                                                    class="dropdown-item">
                                            ...
                                            @endcan
                                            @can('Test Machine Edit')
                                            <li>
                                                <a href="{{ route('test-machine.edit', $testMachine->id) }}"
                                            ...
                                            @endcan
                                            @can('Test Machine Delete')
                                            <li>
                                                <a type="button" class="dropdown-item remove-item-btn"
                                                    wire:click='$dispatch("delete_resource",{{ $testMachine->id }})'>
